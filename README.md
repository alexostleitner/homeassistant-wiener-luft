...
The integration uses the City of Vienna open data datasets [Luftmessnetz: aktuelle Messdaten Wien](https://www.data.gv.at/katalog/datasets/d9ae1245-158e-4d79-86a4-2d9b3defbedc) and [Luftmessnetz: Meteorologie Standorte Wien](https://www.data.gv.at/katalog/datasets/f2bb4f42-2c40-4c9d-890b-c2b3061bfe5a). The integration provides measurements from 18 monitoring stations across Vienna and requires no credentials or API keys.

Available measurements include particulate matter (PM10, PM2.5), nitrogen dioxide (NO₂), nitrogen oxides (NOₓ), ozone (O₃), sulphur dioxide (SO₂), carbon monoxide (CO), air temperature, relative humidity, wind speed, and wind direction. Not every monitoring station measures every parameter, so the set of created sensors can differ between stations. The available sensor types depend on the selected stations and the instruments installed at each location.

During setup, users select which monitoring stations and measurement types they want to include. The integration then creates Home Assistant sensors for the selected combinations that are available in the source data. These sensors update every 30 minutes and can be used like other Home Assistant sensors.

Sensor names are localized in Home Assistant and currently available in English and German.

Data availability, completeness, and correctness depend on the published source data. Individual monitoring stations may occasionally report missing values, delayed updates, or corrected measurements.

### Sensor behavior

If the data source publishes a new station or measurement type, the integration can add the corresponding sensor automatically during a future refresh.
...
When reporting an issue, please include your Home Assistant version, the installed Wiener Luft version, the relevant error log, and the steps needed to reproduce the problem. For UI-related issues, a screenshot is helpful.

## Disclaimer and License

This project is not affiliated with, endorsed by, or maintained by the City of Vienna (Stadt Wien). It only reads and processes the published datasets.
