    ...
    async def test_update_data_logs_unknown_station_codes(self) -> None:
        ...
        with (
            patch.object(
                coordinator_module,
                "fetch_bytes",
                side_effect=[STATION_PAYLOAD, LUMES_FIXTURE.read_bytes()],
            ) as fetch_mock,
            patch.object(coordinator_module.dt_util, "utcnow", return_value=NOW),
        ...
        self.assertIn("STA3", data.measurements.station_codes)
    ...
    async def test_update_data_raises_when_measurement_fetch_fails(self) -> None:
        ...
        with (
            patch.object(
                coordinator_module,
                "fetch_bytes",
                side_effect=[STATION_PAYLOAD, RuntimeError("measurement feed down")],
            ),
            patch.object(coordinator_module.dt_util, "utcnow", return_value=NOW),
            self.assertRaises(coordinator_module.UpdateFailed),
        ):
